-- Atualizar a função get_next_number para usar hierarquia de mensagens
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      NULLIF(selected_number.custom_message, ''),  -- Mensagem do número (se não for vazia)
      NULLIF(group_record.default_message, ''),    -- Mensagem do grupo (se não for vazia)
      'Olá! Vim através do link.'                  -- Mensagem padrão do sistema
    ) as final_message;
END;
$$;

-- Testar a função (opcional - remover após teste)
-- SELECT * FROM get_next_number('seu-grupo-slug');
